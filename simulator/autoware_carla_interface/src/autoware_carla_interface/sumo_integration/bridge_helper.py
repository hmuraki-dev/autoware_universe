...
#
# Vendored into autoware_carla_interface. See NOTICE.md in this directory for
# details. Only the `_vtypes_path` lookup below was adjusted: the original
# upstream layout has `data/vtypes.json` as a sibling of `sumo_integration/`,
# whereas here `data/vtypes.json` is nested inside this package.
...
class BridgeHelper(object):
    ...
    # NOTE (vendoring change): upstream uses "..", "data", "vtypes.json" since
    # data/ is a sibling of sumo_integration/ there. Here data/ is nested
    # inside this package, so no ".." is needed.
    ...
    def get_carla_traffic_light_state(sumo_tl_states):
        ...
        Conversion rules:
        - {'G'}, {'g'}, {'G', 'g'} -> Green
        - one identical state       -> normal SUMO-to-CARLA conversion
        - different states other
            than the G/g combination  -> Red
        - no state                  -> Unknown
        ...
        # Convert a single SUMO state such as 'G' into {'G'}.
        if isinstance(sumo_tl_states, str):
            states = {sumo_tl_states}
        else:
            states = set(sumo_tl_states)

        if not states:
            return carla.TrafficLightState.Unknown

        # New rule:
        # A combination consisting only of G and/or g is displayed
        # as green in CARLA.
        if states.issubset({
        ...
        sumo_tl_state = next(iter(states))
        ...
        if sumo_tl_state == SumoSignalState.YELLOW:
        ...
        if sumo_tl_state == SumoSignalState.OFF:
            return carla.TrafficLightState.Off

        # GREEN_RIGHT_TURN and OFF_BLINKING
        return carla.TrafficLightState.Unknown
