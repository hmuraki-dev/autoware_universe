...
class SumoTLManager(object):
    ...
    def get_state(self, landmark_id):
        """
        Returns the traffic light states of the signals associated with
        the given landmark.

        :return:
            set of SUMO signal states, for example:
                {'G'}
                {'g'}
                {'G', 'g'}
                {'G', 'r'}

            Returns an empty set if no signal is associated.
        """
        states = set()
        ...
        for tlid, link_index in self.get_all_associated_signals(landmark_id):
            ...
            states.add(tl.states[current_phase][link_index])

        # if len(states) > 1:
        #     logging.debug(
        #         '[SumoTLManager] Landmark %s has multiple SUMO states: %s',
        #         landmark_id,
        #         sorted(states)
        #     )
