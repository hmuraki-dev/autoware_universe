...
1. **【解決済み・2026-08-31】CreateID確認の低確率失敗**
   ...
   **根本原因が判明した**: `.inpx`ネットワーク側のステップ時間(当時0.1秒)と、co-simスクリプト側の
   ステップ時間(既定0.05秒)が**不一致**だったことが原因だった。両者のステップ時間を一致させた
   ところ、本現象は発生しなくなることを確認済み。

   > **【訂正・2026-08-31】** この時点では「両者を0.1秒に統一する」方針としていたが、これは誤りで
   > あった。SUMO-CARLA-Autoware連携で実績のあるステップ時間は**0.05秒**であり、0.1秒へ変更する
   > 必要はなかった。訂正後の方針は「`autoware_carla_interface`側の既定値0.05秒を維持し、Vissim
   > `.inpx`側のステップ時間(`simRes`)を0.05秒(`simRes=20`)に合わせて差し替える」である
   > (`.inpx`の差し替えはユーザー側で対応)。本節で確立した「原因はステップ時間の不一致」という
   > 診断結果自体は変わらない。以下2.2節以降の記述はこの訂正を反映済み。

   → **本統合作業への反映**: 2.2節で述べる「`fixed_delta_seconds`と`.inpx`側のステップ時間を
   一致させる」という方針は、単なる推奨(SUMO版に合わせる)ではなく、**このCreateID確認問題を
   回避するために必須の設定**であることが確定した。Vissimネットワーク(`.inpx`)側のステップ時間と
   `autoware_carla_interface`側の`fixed_delta_seconds`・Vissim接続時に渡す`step_length`は
   **必ず一致させること**(片方だけ変更して他方が異なる値のままだと、本現象が再発する
...
- `step_length`(SUMO-CARLA-Autoware連携で実績のある0.05秒を既定値として維持。2.2参照)
...
> **【必須・実機検証で確定、2026-08-31訂正済み】** `autoware_carla_interface`側の`fixed_delta_seconds`
> 既定値0.05秒は**変更せず維持する**(訂正前は「0.1秒に統一する」としていたが誤りだった。
> SUMO-CARLA-Autoware連携で実績のあるステップ時間が0.05秒であるため)。単なる横並びの推奨ではなく、
> **`.inpx`ネットワーク側のステップ時間と`autoware_carla_interface`側の`fixed_delta_seconds`/
> Vissim接続時の`step_length`が一致していないと、0.3節1.で述べたCreateID確認の失敗が発生する
> ことが実機検証で確認済み**である。Vissim`.inpx`側のステップ時間(`simRes`)を0.05秒
> (`simRes=20`)に合わせて差し替えること(`.inpx`の差し替えはユーザー側で対応)。片方だけ変更して
> 他方が異なる値のままだと、本現象が再発するおそれがある。`int(1./0.05)`が意図通り`20`になることも
> 合わせて確認しておくこと(浮動小数点誤差の確認、優先度B)。
...
>   いたが、**原因は`.inpx`側とco-simスクリプト側のステップ時間の不一致であり、両者を一致させる
>   ことで解消済み**である(2026-08-31訂正: 統一先の値は0.1秒ではなく、SUMO-CARLA-Autoware連携
>   実績値の0.05秒)。ただし、ステップ時間を一致させても「数ティックの遅延を伴う
...
- **前提条件(必須)**: 2.2節の通り、`.inpx`側のステップ時間と`autoware_carla_interface`側の
  `fixed_delta_seconds`/`step_length`を0.05秒に一致させること(2026-08-31訂正: 0.1秒ではなく
  0.05秒。SUMO-CARLA-Autoware連携実績値)。これが崩れるとCreateID確認が
...
- 正常終了・異常終了処理の統合(`SimulationSynchronization.close()`との統合、Vissim切断APIの実装
  詳細確認含む)
- **ステップ時間の統一(2.2)** — `.inpx`側と`autoware_carla_interface`側の
  両方を0.05秒に一致させること(2026-08-31訂正: 統一先は0.1秒ではなく、SUMO-CARLA-Autoware
  連携実績値の0.05秒。`autoware_carla_interface`側は元々の既定値のまま変更不要、`.inpx`側を
  ユーザーが差し替え予定)。CreateID確認失敗の既知原因(0.3節1.、解決済み)の再発防止のため、
...
- `fixed_delta_seconds`は既定値`0.05`のまま維持する(2026-08-31訂正: 当初「0.1秒に変更」として
  いたが誤り。SUMO-CARLA-Autoware連携で実績のある0.05秒を維持する方針に訂正した)
- **対象の`.inpx`のシミュレーションステップ時間が0.05秒(`simRes=20`)に設定されていることを
  確認する**(不一致だとStep6でCreateID確認失敗が再発することが判明済みのため、この段階で
  先に確認しておく。`.inpx`側はユーザーが差し替え予定)

### Step 2実施内容(2026-08-31、その後同日中に訂正)
...
> **【訂正・2026-08-31】** 本Stepでは当初、`fixed_delta_seconds`/`max_real_delta_seconds`の
> デフォルトを`0.05`から`0.1`に変更し、`README.md`のTips記載も`0.1`に更新していた。しかし
> これは誤りだった。SUMO-CARLA-Autoware連携で実績のあるステップ時間は**0.05秒**であり、0.1秒へ
> 変更する必要は無かった。同日中に`launch/autoware_carla_interface.launch.xml`・`README.md`
> ともに`fixed_delta_seconds`/`max_real_delta_seconds`のデフォルトを**`0.05`へ差し戻した**
> (現在の状態は「変更なし・既存の0.05を維持」)。Vissim側のステップ時間は、`autoware_carla_interface`
> 側ではなく**Vissim`.inpx`側をユーザーが0.05秒(`simRes=20`)に差し替える**ことで合わせる方針である。

**`.inpxのステップ時間確認`(訂正前に実施した内容。現在は前提が変わっている点に注意)**

- `examples/Town01/Town01.inpx`・`examples/Town03/*.inpx`の`<simulation>`要素を確認したところ、
  いずれも`simRes="10"`(1秒あたり10ステップ = ステップ時間0.1秒)だった。訂正前の「目標0.1秒」との
  一致を確認していたが、訂正後の目標は0.05秒(`simRes=20`)であるため、**この確認結果は無効**。
  ユーザーが`.inpx`側を`simRes=20`に差し替える予定であり、差し替え後に改めて確認が必要
...
2. `ros2 launch autoware_carla_interface autoware_carla_interface.launch.xml --show-args`
   ...
   `fixed_delta_seconds`/`max_real_delta_seconds`が(訂正後の現状)`0.05`、新規Vissim引数が
   すべて意図した説明文・デフォルト値で認識されることを確認
3. `grep`で`.inpx`の`<simulation>`要素の`simRes`/`simPeriod`を確認(上記の通り。ただし
   訂正前時点での確認であり、ユーザーによる`.inpx`差し替え後に再確認が必要)

**未実施(次回以降で実施推奨)**: ユーザーによる`.inpx`のステップ時間差し替え(`simRes=20`)後、
実際にCARLAサーバー・Vissim Kernelを起動してのステップ時間一致確認、および end-to-end 回帰確認
(0.05秒ステップでCARLA単体シミュレーションが従来通り動作すること)。追加したVissim関連`<arg>`は
どの`<param>`にも渡していないため、既存ノードの動作には影響しない構造になっている。
...
- **ステップ時間(`.inpx`と`autoware_carla_interface`側)が0.05秒で一致していることを再確認した上で
  検証する**: 0.3節1.の通り、ここが不一致だとCreateID確認が失敗する現象が再現するため、
  Step2で確認済みであっても本Stepの実機検証開始時にもう一度確認する(2026-08-31訂正:
  目標値は0.1秒ではなく0.05秒)
...
> **【訂正・2026-08-31】** 本 Step 6 の実機テストは`.inpx`側`simRes=10`(0.1秒)/
> `fixed_delta_seconds=0.1`の組み合わせで実施されたものであり、その後の訂正(目標値を0.1秒から0.05秒へ
> 変更)を反映できていない。検証した「ステップ時間を一致させればCreateID確認が成功する」という
> **原理自体は0.05秒でも同様に成立するはず**だが、ユーザーが`.inpx`を`simRes=20`(0.05秒)に
> 差し替え、`autoware_carla_interface`側も同様に0.05秒で接続した上で、**本テストを再実施して
> 0.05秒でも同様に成功することを改めて確認することを推奨する**(未実施)。
...
- (解決済み)EGO登録のCreateID確認が失敗し続ける現象は、`.inpx`とco-simスクリプトのステップ時間
  不一致が原因と判明し、両者を一致させることで解消した(2026-08-31訂正: 統一先の具体的な値は
  0.1秒ではなく、SUMO-CARLA-Autoware連携実績値の**0.05秒**。`autoware_carla_interface`側は既定値
  のまま、`.inpx`側をユーザーが`simRes=20`に差し替える予定)。ステップ時間の一致を実装・運用の
  両方で維持できる仕組み(launchデフォルト値の固定、起動時の不一致検知ログ等)を用意できないか
  検討する(優先度B)
